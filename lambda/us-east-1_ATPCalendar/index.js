...
var URL = "https://s3.amazonaws.com/tennis-calendar/ATPCalendar.ics";
            ...
            ical.fromURL(URL, {}, function (err, data) {
                ...
                if (eventList.length > 0) {
                    ...
                    if (eventDate.startDate && eventDate.endDate) {
                        console.log("startdate" + eventDate.startDate);
                        console.log("enddate" + eventDate.endDate);
                        ...
                        console.log(relevantEvents.length);
...
function getEventsBeweenDates(startDate, endDate, eventList) {
   ...
   for (var i = 0; i < eventList.length; i++) {
        if ((start >= (new Date(eventList[i].start)) && start <= (new Date(eventList[i].end)))
         || (end >= (new Date(eventList[i].start)) && end <= (new Date(eventList[i].end)))
         || (start <= (new Date(eventList[i].start)) && end >= (new Date(eventList[i].end)))) {
